...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // A. PROFILE DETAILS FORM (Updates first_name, last_name, email)
    if (isset($_POST['firstName'])) {
        ...
        if ($stmt->execute()) {
            ...
            $student_name = htmlspecialchars($_SESSION['full_name']); // Re-set for this page load
    ...
    // C. REQUEST CLASS CHANGE FORM
    } elseif (isset($_POST['new_grade'])) {
        $new_grade = $_POST['new_grade'];
        $new_section = $_POST['new_section'];

        // Check if a pending request already exists
        $stmt_check = $conn->prepare("SELECT request_id FROM change_requests WHERE student_id = ? AND status = 'pending'");
        $stmt_check->bind_param("i", $student_id);
        $stmt_check->execute();
        $result_check = $stmt_check->get_result();

        if ($result_check->num_rows > 0) {
            $request_message = "<p class='text-orange-600'>You already have a pending request.</p>";
        } else {
            // Insert new request
            $stmt_insert = $conn->prepare("INSERT INTO change_requests (student_id, new_grade_level, new_section, status) VALUES (?, ?, ?, 'pending')");
            $stmt_insert->bind_param("iss", $student_id, $new_grade, $new_section);
            if ($stmt_insert->execute()) {
                $request_message = "<p class='text-green-600'>Request submitted successfully.</p>";
            } else {
                $request_message = "<p class='text-red-600'>Error submitting request.</p>";
            }
            $stmt_insert->close();
        }
        $stmt_check->close();
...
// 5. --- FETCH CURRENT STUDENT DATA (for pre-filling forms) ---
$stmt = $conn->prepare("SELECT first_name, last_name, email, grade_level, section FROM users WHERE user_id = ?");
$stmt->bind_param("i", $student_id);
$stmt->execute();
$result = $stmt->get_result();
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 bg-white shadow-md flex-shrink-0">
            ...
            <nav class="mt-4 px-2">
                ...
                <a href="student_profile.php" class="flex items-center px-4 py-3 bg-blue-100 text-blue-600 font-semibold rounded-lg">
        ...
        <main class="flex-1 p-6 lg:p-10">
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-md border border-gray-200 max-w-2xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                         ...
                         <div>
                            ...
                            <input type="text" id="grade" name="grade" disabled
                                   value="<?php echo htmlspecialchars($student_data['grade_level']); ?>" 
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-gray-100 text-gray-700 cursor-not-allowed">
                        </div>
                        
                        <div>
                            <label for="section" class="block text-sm font-medium text-gray-700 mb-1">Section</label>
                            <input type="text" id="section" name="section" disabled
                                   value="<?php echo htmlspecialchars($student_data['section']); ?>" 
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-gray-100 text-gray-700 cursor-not-allowed">
                        </div>
                    </div>
                ...
                <form id="requestChangeForm" action="student_profile.php" method="POST" class="border-t pt-6 mt-8">
                    <h2 class="text-xl font-semibold text-gray-900 mb-4">Request Class Change</h2>
                    <p class="text-sm text-gray-600 mb-4">Is your grade or section incorrect? You can request a change here. Your request will be sent to the administrator for approval.</p>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="relative">
                            <label for="newGrade" class="block text-sm font-medium text-gray-700 mb-1">Select New Grade</label>
                            <select id="newGrade" name="new_grade" required
                                ...
                                <option value="" disabled selected>Select a new grade...</option>
                        ...
                        <div class="relative">
                            <label for="newSection" class="block text-sm font-medium text-gray-700 mb-1">Select New Section</label>
                            <select id="newSection" name="new_section" required
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                <option value="" disabled selected>Select a new section...</option>
                    ...
                    <div id="requestMessage" class="text-center mt-4 text-sm font-medium">
                        <?php echo $request_message; ?>
                    </div>
                    <div class="flex justify-end mt-4">
                        <button type="submit" class="bg-orange-500 text-white py-2 px-5 rounded-lg font-semibold shadow-md hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500">
                            Submit Request
                        </button>
                    </div>
                </form>
                ...
                <form id="passwordForm" action="student_profile.php" method="POST">
