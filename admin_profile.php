...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['firstName'])) {
        ...
        if ($stmt->execute()) {
            ...
            $admin_name = htmlspecialchars($_SESSION['full_name']); // Re-set for this page load
...
// 5. --- FETCH CURRENT ADMIN DATA (for pre-filling the form) ---
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-6 lg:p-10">
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-md border border-gray-200 max-w-2xl mx-auto">
                ...
                <form id="profileForm" action="admin_profile.php" method="POST">
                    <h2 class="text-xl font-semibold text-gray-900 mb-4">Profile Details</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="firstName" class="block text-sm font-medium text-gray-700 mb-1">First Name</label>
                            <input type="text" id="firstName" name="firstName" required value="<?php echo htmlspecialchars($admin_data['first_name']); ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>

                        <div>
                            <label for="lastName" class="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                            <input type="text" id="lastName" name="lastName" required value="<?php echo htmlspecialchars($admin_data['last_name']); ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        
                        <div class="md:col-span-2">
                            <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                            <input type="email" id="email" name="email" required value="<?php echo htmlspecialchars($admin_data['email']); ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                ...
                <form id="passwordForm" action="admin_profile.php" method="POST">
                    <h2 class="text-xl font-semibold text-gray-900 mb-4">Change Password</h2>
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label for="currentPassword" class="block text-sm font-medium text-gray-700 mb-1">Current Password</label>
                            <input type="password" id="currentPassword" name="currentPassword" required class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
